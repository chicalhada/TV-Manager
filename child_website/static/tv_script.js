...
function voltarTelaInicial() {
    ...
    // PARAR TUDO - CORREÇÃO DO ÁUDIO
    pararReproducao();
    
    // FORÇAR PARADA DE TODOS OS VÍDEOS
...
function entrarFullscreen(elemento) {
    ...
    // FORÇAR FULLSCREEN - COMPATÍVEL COM TODOS OS BROWSERS
    if (!estaEmFullscreen) {
        var elem = container;
        
        // Tentar fullscreen com diferentes métodos (compatibilidade)
...
function sairFullscreen() {
    // Sair do fullscreen com compatibilidade
...
function reproduzirItem(item, onTerminar) {
    ...
    // REMOVER E PARAR CONTEÚDO ANTIGO
    var oldVideo = container.querySelector('video');
    if (oldVideo) {
        oldVideo.pause();
        oldVideo.currentTime = 0;
...
(async function iniciar() {
    ...
    // LISTENER PARA SAIR DO FULLSCREEN (compatível com todos browsers)
